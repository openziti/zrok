...
#   - zrok2, zrok2-controller, zrok2-frontend, and zrok2-metrics packages installed
...
step_database_postgres() {
    ...
    # RedHat requires explicit initdb before first start
...
INFLUXEOF
    ...
    local setup_status
    setup_status=$(curl -sS "${ZROK2_INFLUX_URL}/api/v2/setup" 2>/dev/null | jq -r '.allowed // true' 2>/dev/null || echo "true")
...
NETWORKEOF
    ...
    # Restart the Ziti controller to pick up the changes
...
step_create_frontend() {
    ...
    if echo "$existing" | grep -q 'public'; then
        ...
        if [[ "$is_dynamic" != *"true"* ]]; then
            ...
            zrok2 admin update frontend "$FRONTEND_TOKEN" --dynamic
        fi
        return
    fi
    ...
    if [[ -z "$FRONTEND_TOKEN" ]]; then
        ...
        FRONTEND_TOKEN=$(zrok2 admin list frontends 2>/dev/null | awk '/public/ {print $1; exit}')
    fi
...
step_dynamic_proxy_controller() {
    ...
    if [[ -f "${identity_dir}/dynamicProxyController.json" ]]; then
        ...
        local source_dir="${HOME}/.zrok2/identities"
...
step_map_namespace_frontend() {
    ...
    local existing
    existing=$(zrok2 admin list namespace-frontend "${ZROK2_NAMESPACE_TOKEN}" 2>/dev/null || true)
    ...
    zrok2 admin create namespace-frontend "${ZROK2_NAMESPACE_TOKEN}" "${FRONTEND_TOKEN}" --default
...
step_tls_permissions() {
    ...
    usermod -aG zrok2-tls zrok2-metrics 2>/dev/null || true
...
step_start_metrics_bridge() {
    info "Step 16: Start zrok2-metrics bridge"
    ...
    # The zrok2-metrics user is added to the zrok2-controller group by postinstall-metrics.bash.
    ...
    # Grant read access to the ctrl.yml and fabric-usage.json for the metrics bridge user
    # via supplementary group membership (ziti-controller group)
    if getent passwd zrok2-metrics &>/dev/null; then
        usermod -aG ziti-controller zrok2-metrics 2>/dev/null || true
    fi

    if systemctl is-active --quiet zrok2-metrics 2>/dev/null; then
        info "zrok2-metrics is already running"
    else
        systemctl enable --now zrok2-metrics
        info "zrok2-metrics bridge started"
    fi
}
