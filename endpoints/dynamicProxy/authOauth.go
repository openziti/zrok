...
func (h *authHandler) tryInlineRefresh(claims *zrokClaims) (string, error) {
	...
	p, ok := globalOAuthRouter.GetProvider(claims.Provider)
	if !ok {
		return "", fmt.Errorf("provider '%v' not found", claims.Provider)
	}
	refresher, ok := p.(sessionRefresher)
	if !ok {
		return "", fmt.Errorf("provider '%v' does not support inline refresh", claims.Provider)
	}
