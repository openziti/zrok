...
func TestShareHandlerOptionsBypassesFrontendAuth(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			restore := stubPublicProxyService(t, tc.serviceCfg)
			defer restore()
			...
			req := httptest.NewRequest(http.MethodOptions, "http://share.example.com/api", nil)
...
func TestShareHandlerGetStillRedirectsForOAuth(t *testing.T) {
	restore := stubPublicProxyService(t, map[string]interface{}{
		"auth_scheme": string(sdk.Oauth),
		"oauth": map[string]interface{}{
			"provider":                     "github",
			"authorization_check_interval": "3h",
	...
	req := httptest.NewRequest(http.MethodGet, "http://share.example.com/api", nil)
	req.Host = "share.example.com"
...
func TestShareHandlerGetStillChallengesBasicAuth(t *testing.T) {
	...
	handler := shareHandler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		upstreamCalled = true
		w.WriteHeader(http.StatusNoContent)
	}), &Config{HostMatch: "example.com"}, nil, nil)
...
func stubPublicProxyService(t *testing.T, proxyConfig map[string]interface{}) func() {
	t.Helper()
