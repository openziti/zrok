...
zrok v2.0.0 represents a major evolution of the platform, introducing a fundamental paradigm shift in how sharing and naming work. This release replaces the reserved sharing model with a more powerful and flexible **namespaces and names** system, adds a new distributed **dynamic proxy** frontend architecture, and includes significant improvements to the zrok Agent.
...
zrok share reserved <token>            # use: zrok share public/private -n <namespace>:<name>
...
- **`defaultFrontend` → `defaultNamespace`**: the configuration key has been renamed and now specifies the default namespace token rather than frontend
  - Update with: `zrok config set defaultNamespace <token>`
...
$ zrok agent status
╭─────────────────┬────────────┬──────────┬─────────────────────╮
│ TOKEN           │ TYPE       │ STATUS   │ ERROR               │
├─────────────────┼────────────┼──────────┼─────────────────────┤
│ err_abc123      │ share      │ failed   │ connection refused  │
│ retry attempt   │ 3/10       │          │ next in 8s          │
╰─────────────────┴────────────┴──────────┴─────────────────────╯
...
Environments:
  - laptop-dev (v2.0.0)
    Names:
      - myapp (public, reserved)
    Shares:
      - myapp.share.zrok.io [public] → http://localhost:8080
```
...
zrok admin add namespaceFrontendMapping <ns> <frontend>         # map to frontend
zrok admin remove namespaceFrontendMapping <ns> <frontend>      # remove mapping
zrok admin list namespaceFrontendMappings <namespaceToken>      # list frontends
zrok admin list frontendNamespaceMappings <frontendToken>       # list namespaces
```
...
| v2.0 Controller | ⚠️ Limited (no namespaces) | ✅ Full support |

**Important**: v2.0 clients cannot connect to v1.x controllers. v1.x clients can connect to v2.0 controllers but will not have access to namespace features.
