...
To accomplish this, we're going to run 3 separate `zrok access private` commands on our VPS (see the [frontdoor guide](../frontdoor.mdx), or [zrok-private-access Docker Compose guide](../docker-share/docker_private_share_guide.md#access-the-private-share) for details on an approach for setting this up). One command each for shares `A`, `B`, and `C`. The `zrok access private` command works like this:
