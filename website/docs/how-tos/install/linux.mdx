...
:::info
Check out [zrok frontdoor](../frontdoor.mdx?os=Linux) for running `zrok` as an always-on service.
:::
...
[An Arch User Repository (AUR) package](https://aur.archlinux.org/packages/zrok-bin) is maintained by the Arch community. As of April 2025, the AUR package includes the `zrok2` CLI and [the `zrok2-agent.service` systemd `--user` service](/how-tos/agent/linux-service.mdx).
