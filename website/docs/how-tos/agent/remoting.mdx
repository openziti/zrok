...
Enrolling an Agent in remoting requires an enabled environment. You can use the `zrok2 agent enroll` command from an enabled environment to enroll your agent:
...
The `zrok2 agent unenroll` command will remove all remote control access from an Agent in an environment:
