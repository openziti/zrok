---
title: Configure HTTP health checks
...
Then the new `zrok2 agent share http-healthcheck` command can be used to validate that the Agent is able to communicate with the backend target:
