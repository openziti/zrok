---
title: Share with Docker
sidebar_label: Share with Docker
---
...
This page provides `docker` and `docker compose` examples of mounting the host's `zrok` environment on the container. You'll need to first [enable zrok on the Docker host](../../get-started/index.mdx#installing-the-zrok-command) to use this approach.

## Create a permanent public share
...
1. Reserve a subdomain by running `zrok2 reserve public --unique-name "toaster" https:toasterpp:8080` on the Docker host.
...
## Create a temporary public share
...
docker.exe run `
    --rm `
    --network "host" `
    --volume "${env:USERPROFILE}\.zrok2:/.zrok2" `
    --user "1000" `
    openziti/zrok share public `
        --headless `
        https://10.11.12.13:8080
...
docker.exe run ^
    --rm ^
    --network "host" ^
    --volume "%USERPROFILE%\.zrok2:/.zrok2" ^
    --user "1000" ^
    openziti/zrok share public ^
        --headless ^
        https://10.11.12.13:8080
...
docker run \
    --rm \
    --network "host" \
    --volume "/mnt/c/Users/$(powershell.exe -Command 'Write-Output $env:USERNAME' | tr -d '\r')/.zrok2:/home/ziggy/.zrok2" \
    --user "${UID:-1000}:${GID:-1000}" \
    openziti/zrok share public \
        --headless \
        https://10.11.12.13:8080
...
1. `--volume ~/.zrok2:/home/ziggy/.zrok2` mounts the `zrok` configuration from the Docker host into the container.
