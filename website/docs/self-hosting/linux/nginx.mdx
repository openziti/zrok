---
sidebar_position: 50
sidebar_label: NGINX reverse proxy for zrok
---
...
I'll assume you have a running `zrok` controller and frontend and wish to front both with NGINX providing server TLS. Go back to [Self-Hosting Guide](./index.mdx) if you still need to spin those up.
...
List available frontends to obtain the token identifier of the frontend named "public". You may need to set `ZROK2_ADMIN_TOKEN` or `ZROK2_API_ENDPOINT` before running `zrok2 admin`.

```bash
$ zrok2 admin list frontends
...
```bash
$ zrok2 admin update frontend 2NiDTRYUww18 --url-template https://{token}.zrok.quigley.com:443
