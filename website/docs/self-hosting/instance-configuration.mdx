...
sidebar_label: Use another zrok instance
---

:::note
This guide is relevant if you are self-hosting or using a friend's zrok instance instead of using zrok-as-a-service
from `zrok.io`.
:::
...
Use `zrok2 status` to inspect the state of your local _environment_. In zrok, an _environment_ is each shell where
