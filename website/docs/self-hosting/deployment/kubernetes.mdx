---
title: Deploy zrok on Kubernetes
sidebar_label: Kubernetes
sidebar_position: 55
---

This guide deploys a self-hosted zrok2 instance in Kubernetes using the `zrok2` Helm chart. The chart creates a
controller, frontend, and bootstrap job that connect to an existing OpenZiti overlay network.
...
- `helm` 3.x installed
- An OpenZiti controller already deployed (e.g., via the [`ziti-controller` Helm
  chart](https://github.com/openziti/helm-charts/tree/main/charts/ziti-controller))
- [Traefik](https://doc.traefik.io/traefik/getting-started/install-traefik/) ingress controller (recommended;
  ingress-nginx is sunset/EOL)
- A DNS zone with a wildcard `*` A record pointing to your ingress controller (e.g., `*.share.example.com`)

## Step 1: Add the Helm repository

Add the OpenZiti Helm repository and update your local index:
...
By default, the chart uses SQLite3 with a PersistentVolumeClaim. For production, configure an external PostgreSQL
database:
...
When `postgresql.host` is set, the chart uses PostgreSQL and the SQLite3 PVC isn't created.
...
Install the `zrok2` chart into a new `zrok2` namespace:
...
- A **controller deployment** with an init container that runs `zrok2 admin bootstrap`
- A **frontend deployment** with an init container that creates the default "public" frontend in OpenZiti and zrok2
...
The chart creates Ingress resources for the controller and frontend when enabled.
[Traefik](https://doc.traefik.io/traefik/) is the recommended ingress controller (`ingress-nginx` is sunset/EOL). For
TLS, use [cert-manager](https://cert-manager.io/) with a `ClusterIssuer` that supports DNS-01 challenges (required for
...
The bootstrap init container waits for the Ziti controller CA ConfigMap to exist in the zrok2 namespace. If you're using
the `ziti-controller` chart with trust-manager, the CA bundle is propagated automatically to all namespaces. Verify:
...
If you see a ConfigMap with a different name (e.g., `ziti-controller1-ctrl-plane-cas`), update `ziti.caSecretName` in
your values to match.

### Frontend pod stuck in ContainerCreating

The frontend needs the `zrok2-frontend-identity` Secret, which is created by the controller's init container. Wait for
the controller pod to complete its init phase:
...
The chart source and full values reference are available in the
[openziti/helm-charts](https://github.com/openziti/helm-charts/tree/main/charts/zrok2) repository.
