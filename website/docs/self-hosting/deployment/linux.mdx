...
This guide deploys all zrok components—controller, frontend, and metrics bridge—on a single Linux server. This is the
simplest production-ready configuration. To scale the frontend for higher throughput or availability, see [Scaling frontends](@zrokdocs/self-hosting/scaling-frontends).
:::
...
OpenZiti provides secure network backhaul for zrok public and private shares. You need an OpenZiti controller and an
OpenZiti router. You can run everything on the same Linux VPS.

Follow the [OpenZiti Linux deployment guides](https://netfoundry.io/docs/openziti/category/linux/) to install and
configure the OpenZiti controller and OpenZiti router on your server. Once both services are running, verify the router
is online:
...
Follow [the Linux installation guide](@zrokdocs/how-tos/install/linux) to install the `zrok2` package from the
repository or manually install the binary for your platform.
...
The `admin` section defines privileged administrative credentials. Set the same value in the `ZROK2_ADMIN_TOKEN`
environment variable to run `zrok2 admin` commands.

The `bridge` section configures the metrics bridge to consume OpenZiti `fabric.usage` events from a file and publish
them to the AMQP queue.

The `metrics` section configures the controller to consume events from the AMQP queue and write them to InfluxDB.

The `dynamic_proxy_controller` section enables the gRPC/AMQP system for dynamic frontends. The identity file referenced
here will be created in a later step.

:::note

- See the [reference configuration at `etc/ctrl.yml`](https://github.com/openziti/zrok/blob/main/etc/ctrl.yml) for all
configuration options.
- See the separate guides on [Configure metrics](@zrokdocs/self-hosting/metrics-and-limits/configuring-metrics) and
[Configure limits](@zrokdocs/self-hosting/metrics-and-limits/configuring-limits) for details about these specialized
...
[Read more about configuring your self-hosted zrok instance](@zrokdocs/self-hosting/instance-configuration).
...
overlay via AMQP. Without it, the frontend cannot route named shares (e.g., `myapp.zrok.example.com`) — only
random-token shares would work with polling.
...
routing `myapp.zrok.example.com` to the share's backend — no polling delay. The `dynamicProxyController` Ziti service is
the gRPC channel over the Ziti overlay that delivers these mapping updates securely.
:::
...
The `amqp_subscriber` and `controller` sections connect the frontend to the dynamicProxyController gRPC service (Step 7)
for real-time mapping updates. The `host_match` value must match the namespace name (Step 8).
...
If the TLS certificate files are only readable by root (common with Let's Encrypt), grant read access to the service
users:
...
For a complete reference of all frontend options including OAuth, see the [Dynamic proxy frontend migration guide](@zrokdocs/self-hosting/dynamic-proxy).
...
The zrok metrics pipeline starts at the OpenZiti controller, which emits `fabric.usage` events. Add the following to
your OpenZiti controller configuration:
...
For more details, see [Configure metrics](@zrokdocs/self-hosting/metrics-and-limits/configuring-metrics).
...
See [Configure limits](@zrokdocs/self-hosting/metrics-and-limits/configuring-limits) to enforce bandwidth and resource
...
`zrok2 create name` step registers the name in the namespace — this is the v2 equivalent of `zrok reserve` in v1.
...
The service reads the `bridge` section from `/etc/zrok2/ctrl.yml`:
...
For dynamic frontend troubleshooting (AMQP connectivity, gRPC errors, mapping issues), see the [Dynamic Proxy Frontend
Guide](@zrokdocs/self-hosting/dynamic-proxy#troubleshooting).

For metrics troubleshooting (InfluxDB connectivity, AMQP queues, event flow), see [Configuring
Metrics](@zrokdocs/self-hosting/metrics-and-limits/configuring-metrics).
