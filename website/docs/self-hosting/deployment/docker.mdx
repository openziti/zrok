...
[Deploy zrok on Linux](@zrokdocs/self-hosting/deployment/linux) — OpenZiti overlay, zrok2
...
On first start, one-shot init services bootstrap the OpenZiti overlay and zrok2
...
The OpenZiti router data-plane port (3022) already binds to `0.0.0.0` because SDK clients must reach it
from outside the Docker network and OpenZiti uses TLS for security.
...
[Deploy zrok on Linux](@zrokdocs/self-hosting/deployment/linux#step-12-configure-openziti-metrics-events)
...
The `zrok2-init` service waits for the OpenZiti controller health check. If the
