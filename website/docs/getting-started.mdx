...
zrok shares are _ephemeral_ unless you specifically create a "persistent" share.
...
zrok supports creating _shares_ that have a consistent _share token_ that survives restarts of the `zrok2 share` command. These are considered _non-ephemeral_, and are called _persistent shares_.
