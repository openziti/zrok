...
You create a new _account_ with NetFoundry's zrok _instance_ by subscribing in [myzrok.io](https://myzrok.io) or in a self-hosted zrok _instance_ by running [the `zrok2 invite` command](../self-hosting/self-service-invite.mdx) or the `zrok2 admin create account` command.
