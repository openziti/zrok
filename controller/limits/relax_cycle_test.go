...
func newRelaxFixture(t *testing.T) *relaxFixture {
	...
	agent, err := NewAgent(DefaultConfig(), &metrics.InfluxConfig{Url: "http://localhost:8086"}, &zrokEdgeSdk.Config{}, &emailUi.Config{}, str)
	require.NoError(t, err)
	agent.ifx = fixedBandwidthReader{}
	agent.setZitiFactory(func() (*rest_management_api_client.ZitiEdgeManagement, error) {
...
func TestRelaxRetainsBadShareAndCompletesOtherAccount(t *testing.T) {
	...
	policy, err := zrokEdgeSdk.FindServicePolicyByName("good@example.com-frontend-good@example.com-env-good@example.com-share-dial", f.fake.Edge())
	require.NoError(t, err)
