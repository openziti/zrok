...
func (str *Store) CreateAccessGrant(shareId, accountId int, trx *sqlx.Tx) (int, error) {
	stmt, err := trx.Prepare("insert into access_grants (share_id, account_id) values ($1, $2) returning id")
	if err != nil {
...
func (str *Store) IsAccessGrantedToAccountForShare(shrId, acctId int, trx *sqlx.Tx) (int, error) {
	count := 0
	err := trx.QueryRowx("select count(0) from access_grants where share_id = $1 and account_id = $2 and not deleted", shrId, acctId).Scan(&count)
	if err != nil {
...
func (str *Store) DeleteAccessGrantsForShare(shrId int, trx *sqlx.Tx) error {
	stmt, err := trx.Prepare("update access_grants set updated_at = current_timestamp, deleted = true where share_id = $1")
	if err != nil {
...
func (str *Store) DeleteAccessGrantsForShareAndAccount(shrId, acctId int, trx *sqlx.Tx) error {
	stmt, err := trx.Prepare("update access_grants set updated_at = current_timestamp, deleted = true where share_id = $1 and account_id = $2")
	if err != nil {
