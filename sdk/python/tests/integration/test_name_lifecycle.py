...
class TestNameLifecycle:
    def test_list_namespaces(self, enabled_root):
        ...
        assert len(nss) >= 1
        tokens = [ns.NamespaceToken for ns in nss]
    ...
    def test_create_and_delete_name(self, enabled_root):
        ...
        # Get the public namespace by token (Name is the DNS zone, not "public")
        nss = list_namespaces(enabled_root)
        public_ns = next(ns for ns in nss if ns.NamespaceToken == "public")
